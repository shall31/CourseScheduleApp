...
class DataRepository(private val dao: CourseDao) {
    ...
    fun getNearestSchedule(queryType: QueryType) : LiveData<Course?> =
        dao.getNearestSchedule(QueryUtil.nearestQuery(type = queryType))
    ...
    fun getTodaySchedule() : List<Course> {
        val daysWeek = Calendar.getInstance().get(Calendar.DAY_OF_WEEK)
        return dao.getTodaySchedule(daysWeek)
    }
